...
export function AppDeviceSettings(){
  ...
  return <div className="app-device-settings">
    <div className="app-device-row"><Download/><div><strong>{nl?'Webapp installeren':'Install Web App'}</strong><span>{pwa.isStandalone?(nl?'App is al geïnstalleerd':'App already installed'):pwa.canInstall?(nl?'Installatie beschikbaar op dit apparaat.':'Installation is available on this device.'):(nl?'Installatie is niet beschikbaar in deze browser.':'Installation is unavailable in this browser.')}</span></div>{pwa.isStandalone?<span className="badge badge-success"><Check/>{nl?'Geïnstalleerd':'Installed'}</span>:pwa.canInstall?<Button variant="secondary" onClick={()=>pwa.install()}><Download/>{nl?'Installeren':'Install'}</Button>:null}</div>
