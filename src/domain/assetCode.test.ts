...
describe('normalizeAssetCode',()=>{it.each([['KCSMD01','KCSMD-01'],['kcsmd 001','KCSMD-01'],['KCSL-125','KCSL-125'],['KCSBD 5000','KCSBD-5000'],['KCSL-10001','KCSL-10001']])('normalizes %s', (input,expected)=>expect(normalizeAssetCode(input)?.fullAssetCode).toBe(expected));it('code groups are unlimited: a code above the old 5000 cap is valid',()=>expect(normalizeAssetCode('KCSMD-5001')).not.toBeNull());it.each(['KCSMD-0','KCSXYZ-01','KCSL-ABC'])('rejects %s',input=>expect(normalizeAssetCode(input)).toBeNull())});
