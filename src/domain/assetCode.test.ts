...
describe('allocateAssetCodeNumber',()=>{
 const group={minimumNumber:1,maximumNumber:5000,nextAvailableNumber:31};
 ...
 it('TEST 3/self-healing: derives the floor from real asset history even if the counter drifted behind it',()=>{
  const staleGroup={minimumNumber:1,maximumNumber:5000,nextAvailableNumber:5};
 ...
 it('rejects once the code group range is exhausted',()=>{
  const fullGroup={minimumNumber:1,maximumNumber:31,nextAvailableNumber:32};
  expect(allocateAssetCodeNumber(fullGroup,[],'KCSDB')).toEqual({error:'range-exhausted'});
 });
});
