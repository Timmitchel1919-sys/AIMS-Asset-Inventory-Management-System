...
// New conditions share the rank of their closest existing one so existing assessments are unchanged; Unknown cannot be compared.
const conditionRank:Record<string,number>={New:0,Excellent:1,Good:2,Fair:3,Poor:4,Bad:4,Defective:5,'Out of service':5,'Beyond Repair':6,'Use for parts':6};
export function conditionAssessment(before:string,after:string){if(!(before in conditionRank)||!(after in conditionRank))return'Requires inspection';const change=conditionRank[after]-conditionRank[before];return change<=0?'No deterioration':change===1?'Normal wear':change===2?'Minor damage':change>=4?'Major damage':'Requires inspection'}
...
export const validThemes:ThemeId[]=['aimsAzureGlass','dark','aimsEmeraldGloss','aimsLight'];
export const normalizeTheme=(value:string|null):ThemeId=>{
 ...
 if(value==='aimsMidnight'||value==='aims-midnight'||value==='midnight-blue')return'dark';
