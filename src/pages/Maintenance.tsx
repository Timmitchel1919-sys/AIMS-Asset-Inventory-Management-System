...
export default function MaintenancePage(){
  const {language,formatDate}=useApp(),nl=language==='nl',snapshot=useMockSnapshot(),repository=useRepository();
  ...
  return <OfflineGate><div className="page"><PageHeader title={nl?'Preventief onderhoud':'Preventive maintenance'} description={nl?'Planning, checklists, herhaling, uitvoering en achterstallige taken.':'Schedules, checklists, recurrence, execution and overdue tasks.'} actions={<><Button className="page-header-secondary-action" variant="secondary" onClick={()=>setCalendar(value=>!value)}><CalendarDays/>{nl?'Kalender':'Calendar'}</Button><Button onClick={()=>setCreate(true)}><Plus/>{nl?'Planning toevoegen':'Add schedule'}</Button></>}/>
    {calendar&&<section className="card"><h2>{nl?'Onderhoudskalender':'Maintenance calendar'}</h2><div className="calendar-strip">{rows.sort((a,b)=>a.nextDate.localeCompare(b.nextDate)).map(item=><button key={item.id} onClick={()=>setSelected(item)}><small>{formatDate(item.nextDate)}</small><b>{item.assetCode}</b><span>{item.type}</span></button>)}</div></section>}
