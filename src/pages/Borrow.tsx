...
export default function BorrowPage({overdue=false}:{overdue?:boolean}={}){
  ...
  return <OfflineGate><div className="page">
    ...
    <Dialog open={!!preview} title={preview==='receipt'?(nl?'Retourbewijs':'Return receipt'):(nl?'Uitleenovereenkomst':'Borrow agreement')} description={current?.reference} onClose={()=>setPreview(null)}><div className="print-preview"><header><strong>AIMS Asset & Inventory Management System</strong><h2>{preview==='receipt'?'Return receipt':'Equipment loan agreement'}</h2></header><p><b>{current?.borrower}</b> â€” {current?.department}</p><p>{current?.assetCode} Â· {current?.asset}</p><p>{current?.borrowDate} â†’ {current?.dueDate}</p><p>{nl?'Handtekening lener':'Borrower signature'}: {current?.borrowerSignature||'________________'}</p><p>{nl?'Handtekening medewerker':'Staff signature'}: {current?.staffSignature||'________________'}</p><footer><span>{nl?'Schoolinformatie: officiÃ«le gegevens nog niet geleverd':'School information: official details not supplied'}</span><span>1 / 1</span></footer></div></Dialog>
  </div></OfflineGate>;
}
