...
export default function DownloadPage(){
  ...
  const installLabel=pwa.isStandalone?(nl?'App is al geïnstalleerd':'App already installed'):pwa.canInstall?(nl?'Webapp installeren':'Install Web App'):(nl?'Installatie-instructies':'Installation instructions');
  return <main className="download-page"><div className="download-page__shell">
    ...
    <section className="download-options" aria-label={nl?'Downloadopties':'Download options'}>
      <article><Globe2/><h2>{nl?'Webapp':'Web App'}</h2><p>{nl?'Werkt in moderne browsers op desktop, tablet en mobiel.':'Works in modern browsers on desktop, tablet, and mobile.'}</p><button className="btn btn-primary" disabled={pwa.isStandalone} onClick={()=>pwa.canInstall?pwa.install():pwa.showGuidance()}><Download/>{installLabel}</button></article>
