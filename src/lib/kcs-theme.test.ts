...
describe('authenticated AIMS theme persistence',()=>{
  ...
  it('persists AIMS Midnight through the existing preference',()=>{applyKcsTheme('dark');expect(localStorage.getItem(AUTHENTICATED_THEME_STORAGE_KEY)).toBe('dark');expect(document.documentElement.dataset.theme).toBe('aims-midnight');expect(getStoredKcsTheme()).toBe('dark')});
