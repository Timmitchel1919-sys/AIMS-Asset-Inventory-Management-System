...
test('public authentication restores the saved theme and the signed-in sidebar renders', async ({ page }) => {
  await page.addInitScript(() => { localStorage.setItem('kcs-auth', 'out'); localStorage.setItem('kcs-theme', 'kcs-forest-gold'); });
  await page.goto('/login');
  await expect(page.locator('html')).toHaveAttribute('data-theme', 'emerald-gloss');
