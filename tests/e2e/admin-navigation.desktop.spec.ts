...
test('admin navigation is consolidated and responsive',async({page})=>{
  ...
  await expect(page.locator('.app-sidebar nav a[href="/admin"]')).toHaveCount(0);
  ...
  expect(finalNavigation).toEqual(['/disposals','/settings','/assistant']);
  await expect(page.locator('.app-sidebar')).not.toContainText('Download app');
