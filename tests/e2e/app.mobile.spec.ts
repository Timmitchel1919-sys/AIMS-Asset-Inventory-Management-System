...
test.describe('KCS mobile acceptance',()=>{
  test('drawer navigation, mobile cards and touch layout work without overflow',async({page})=>{
    ...
    await page.goto('/dashboard');await page.getByRole('button',{name:/more/i}).click();const drawer=page.getByRole('complementary',{name:/primary navigation/i});await expect(drawer).toBeVisible();await drawer.getByRole('link',{name:/ICT Assets|ICT-middelen/}).click();await expect(page).toHaveURL(/\/assets$/);await expect(page.locator('.mobile-records article').first()).toBeVisible();
  });
